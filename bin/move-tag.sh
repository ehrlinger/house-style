...
#
# Moving this tag is the propagation event: nine repositories pin it, and the
...
if [ -z "$message" ]; then
  ...
  echo "The message is not optional: it is the only record of why nine repos went red." >&2
  exit 1
fi
...
git merge-base --is-ancestor "$sha" origin/main \
  || fail "$(git rev-parse --short "$sha") is not an ancestor of origin/main.
         Tagging an unpushed commit points nine repositories at a commit they
...
echo "    note  $message"
echo ""
echo "This makes all nine governed repositories report drift until each recomposes."
